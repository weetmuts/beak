...
int ReverseTarredFS::parseTarredfsContent(vector<char> &v, string tar_path) {
    ...
    if (type != "#tarredfs 0.1") {
        debug(REVERSE,  "Type was not \"#tarredfs 0.1\" as expected! It was \"%s\"\n", type.c_str());
        return ERR;
    }
...
void ReverseTarredFS::loadTaz(string taz_path, string path) {
    ...
    if (n != "tarredfs") {
        debug(REVERSE,  "reverse", "Volume header is not tarredfs %s", taz_path.c_str());
        tar_close(t);    
        return;
    }
    ...
    if (n != "tarredfs-contents") {
        debug(REVERSE,  "reverse", "Second entry in %s is not tarredfs-contents but %s", taz_path.c_str(), n.c_str());
        tar_close(t);    
        return;
    }
    ...
    for (size_t i=0; i<size; i += T_BLOCKSIZE)
    {
        ...
        if (k != T_BLOCKSIZE)
        {
            if (k != -1) { errno = EINVAL; }
            tar_close(t);
            debug(REVERSE, "reverse", "Internal error reading block from taz file.");
            return;
        }
    ...
    if (rc) {
        debug(REVERSE, "reverse", "Could not parse the tarredfs-contents file in %s\n", taz_path.c_str());
        tar_close(t);
        return;
    }
...
int ReverseTarredFS::readdirCB(const char *path_char_string, void *buf, fuse_fill_dir_t filler,
...
int ReverseTarredFS::readCB(const char *path_char_string, char *buf, size_t size, off_t offset_, struct fuse_file_info *fi)
{
    ...
    try {
        ...
        if (fd==-1) {
            failure(REVERSE, "Could not open file >%s< in underlying filesystem err %d", tar.c_str(), errno);
            throw;
        }
        debug(REVERSE, "reverse", "Reading %ju bytes from offset %ju in file %s\n", size, offset, tar.c_str());
        l = pread(fd, buf, size, offset);
        close(fd);        
        if (l==-1) {
            failure(REVERSE, "Could not read from file >%s< in underlying filesystem err %d", tar.c_str(), errno);
            throw;
        }
        rc = l;
    ok:
