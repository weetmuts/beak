...
template<ReverseTarredFS*fs> struct Getattr {
    static int cb(const char *path, struct stat *stbuf) {
        return fs->getattrCB(path, stbuf);
    }
};

template<ReverseTarredFS*fs> struct Readdir {
...
template<ReverseTarredFS*fs> struct Read {
...
int reversemain(int argc, char *argv[])
{
    ...
    tarredfs_ops.readlink = Readlink<&curr>::cb;
    ...
    struct stat sb;
    string taz = curr.root_dir+"/taz00000000.tar";
    int rc = stat(taz.c_str(), &sb);
    if (rc || !S_ISREG(sb.st_mode)) {
