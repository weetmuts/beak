...
int
size_to_oct_nonull(size_t num, char *oct, size_t octlen)
{
    char buf[octlen+2];
    size_t check;

    snprintf(oct, octlen, "%*zo", (int)(octlen - 1), (size_t)num);
    oct[octlen - 1] = ' ';

    memcpy(buf, oct, octlen);
    buf[octlen+1] = 0;

    // Check that the integer was succesfully stored.
    check = (size_t)strtol(buf, NULL, 8);
    if (check != num) {
        // Nope! Return error!
        return 1;
    }
