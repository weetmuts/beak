...
th_set_path(TAR *t, const char *pathname)
{
	...
	else if (strlen(pathname) > T_NAMELEN-1)
	{
		/* POSIX-style prefix field */
                const char *start_search = &(pathname[strlen(pathname) - T_NAMELEN]);
                const char *end_search = &(pathname[strlen(pathname)]);

                while (start_search < end_search) {
                    tmp = strchr(start_search, '/');
                    if (tmp == NULL)
                    {
			fprintf(stderr, "!!! '/' not found in \"%s\"\n", pathname);
			return;
                    }
                    if (end_search - tmp < T_NAMELEN-2) {
                        break;
                    }
                    start_search = tmp+1;
                }
                size_t c = strlen(&(tmp[1]))+strlen(suffix);
		size_t r = snprintf(t->th_buf.name, 100, "%s%s", &(tmp[1]), suffix);
...
th_set_from_stat(TAR *t, struct stat *s)
{
	...
	rc = th_set_mtime(t, s->st_mtime);
        if (rc) { err |= TH_COULD_NOT_SET_MTIME; }
	if (S_ISREG(s->st_mode)) {
            rc = th_set_size(t, s->st_size);
            if (rc) { err |= TH_COULD_NOT_SET_SIZE; }
        }
	else {
		th_set_size(t, 0);
        }
