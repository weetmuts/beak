...
#define th_set_mtime(t, fmtime) \
        size_to_oct_nonull((fmtime), (t)->th_buf.mtime, 12)
#define th_set_size(t, fsize) \
	size_to_oct_nonull((fsize), (t)->th_buf.size, 12)

#define TH_COULD_NOT_SET_MTIME 1
#define TH_COULD_NOT_SET_SIZE 2

/* encode everything at once (except the pathname and linkname)
   if the returned values differs from zero, there was an error.
   check the bits defined in TH_COULD_NOT_SET_... above */
int th_set_from_stat(TAR *t, struct stat *s);
...
#define int_to_oct(num, oct, octlen) \
	snprintf((oct), (octlen), "%*zo ", (octlen) - 2, (size_t)(num))

/* integer to string-octal conversion, no NULL */
int size_to_oct_nonull(size_t num, char *oct, size_t octlen);
