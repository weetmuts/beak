...
void parseForwardOptions(int *argc, char **argv, TarredFS *tfs,
        struct fuse_operations *tarredfs_ops)
{
    for (int i = 1; argv[i] != 0; ++i)
    {
        ...
        else if (!strcmp(argv[i], "-d") || !strcmp(argv[i], "--fuse-debug"))
        {
            setLogLevel(DEBUG);
        }
        ...
        else if (!strcmp(argv[i], "-i") || !strcmp(argv[i], "--include"))
        {
            ...
            tfs->filters.push_back(
                    pair<Filter, regex_t>(Filter(argv[i + 1], INCLUDE), re));
        ...
        else if (!strcmp(argv[i], "-x"))
        {
            ...
            tfs->filters.push_back(
                    pair<Filter, regex_t>(Filter(argv[i + 1], EXCLUDE), re));
        ...
        else if (!strcmp(argv[i], "-ta"))
        {
            ...
            {
                error(MAIN,
                        "Cannot set -ta because \"%s\" is not a proper number (e.g. 1,2K,3M,4G,5T)\n",
                        argv[i + 1]);
            }
        ...
        else if (!strcmp(argv[i], "-tr"))
        {
            ...
            {
                error(MAIN,
                        "Cannot set -tr because \"%s\" is not a proper number (e.g. 1,2K,3M,4G,5T)\n",
                        argv[i + 1]);
            }
        ...
        {
            tfs->setTarListFile(argv[i + 1]);
            debug(MAIN, "Store tar list in \"%s\"\n", argv[i + 1]);
            eraseArg(i, argc, argv);
            eraseArg(i, argc, argv);
            i--;
        }
        else if (!strcmp(argv[i], "-p"))
        {
            ...
            {
                error(MAIN, "main",
                        "Cannot set forced tar collection depth to a negative number.");
            }
        ...
        else if (!strcmp(argv[i], "-s") && argv[i + 1] != NULL)
        {
            ...
            {
                error(MAIN,
                        "Cannot set -s bandwidth because \"%s\" is not a proper number (e.g. 1,2K,3M,4G,5T)\n",
                        bws.c_str());
            }
            if (rc2)
            {
            ...
            info(MAIN,
                    "Calculated target tar size %sB and trigger size %sB from %s requests per second over %sbit/s .\n",
                    ta.c_str(), tr.c_str(), rqss.c_str(), bws.c_str());
            eraseArg(i, argc, argv);
            eraseArg(i, argc, argv);
            i--;
        }
...
void parseReverseOptions(int *argc, char **argv, ReverseTarredFS *tfs,
        struct fuse_operations *tarredfs_ops)
{
    for (int i = 1; argv[i] != 0; ++i)
    {
        ...
        else if (!strncmp(argv[i], "-q", 2) || !strcmp(argv[i], "--quiet"))
        {
            setLogLevel(QUITE);
            eraseArg(i, argc, argv);
            i--;
...
{
    static int add(const char *fpath, const struct stat *sb, int tflag,
            struct FTW *ftwbuf)
    {
...
{
    static int cb(const char *path, char *buf, size_t size, off_t offset,
...
int main(int argc, char *argv[])
{
    ...
    // Store the list of tars in a text file. Used for diffs.
    forward_fs.saveTarListFile();
